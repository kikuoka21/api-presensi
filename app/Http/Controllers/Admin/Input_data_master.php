                    ...
                    if ($user->chek_token($username, $token, $type)) {
                        if ($user->getakses_admin($username) && $user->getakses_admin_piket($username)) {
                            ...
                            if (!$hasil) {
                                ...
                                $inputmaster->input_wali($json->nis, md5('%' . md5($json->tgl_lhr) . ' secret keynya ad@l@h al-kamal!'));
                                $result = [
                                    'code' => 'OK4'
                                ];
                            } else
    ...
    {
        {
            ...
            if ($tool->IsJsonString($json)) {
                ...
                if (isset($json->token) && isset($json->x1d) && isset($json->type) && isset($json->key) &&
                    ...
                    $result = ['code' => 'format data yg dikirim salah' . isset($json->token) . isset($json->x1d) . isset($json->type) . isset($json->key) .
