                    ...
                    if ($user_wali->chek_token_wali($username, $token)) {
                        ...
                        if ($tag1 && $tag2) {
                            ...
                            if (!$tag2) {
                                ...
                                $ketzz = 'Tidak terdaftar pada kelas';
                            }
                            $status_siswa = [
                                'status' => '',
                                'ket' => $ketzz
...
//
    ...
    public function presensi(Request $request)
    {
        ...
        if ($tool->IsJsonString($json)) {
            $json = json_decode($json);
            if (isset($json->token) && isset($json->x1d) && isset($json->tanggal)) {
                $token = $json->token;
                $username = $json->x1d;
...
//                if (true) {
                if ($user_wali->chek_token_wali($username, $token)) {
                    ...
                    $datakls = [
                        'nama' => '-',
                        'thn_ajar' => $tool->thn_ajar_pertanggal($json->tanggal),
                        'ketua' => '-',
                        'wali' => '-'
                    ];
